      ...
      <div className="group relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-slate-900/60 via-slate-900/40 to-slate-900/60 backdrop-blur-xl hover:border-purple-500/30 transition-all duration-500">
        ...
        <table className="w-full text-sm">
          <thead className="bg-slate-900/60 border-b border-white/5">
            <tr>
              <th className="text-left px-4 py-3 font-medium text-slate-400 uppercase tracking-wider text-xs">Ranking</th>
              <th className="text-left px-4 py-3 font-medium text-slate-400 uppercase tracking-wider text-xs">Nombre</th>
              <th className="text-right px-4 py-3 font-medium text-slate-400 uppercase tracking-wider text-xs">Cantidad</th>
              <th className="text-right px-4 py-3 font-medium text-slate-400 uppercase tracking-wider text-xs">Total</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-white/5">
            {data.map((r, idx) => (
              <tr key={idx} className="hover:bg-white/5 transition-colors duration-200 group/row">
                ...
                <td className="px-4 py-3 text-slate-200 font-medium truncate max-w-[200px] group-hover/row:text-white transition-colors">
                  {r[nameKey] || r.producto || r.cliente || r.categoria}
                </td>
                <td className="px-4 py-3 text-right">
                  <span className="inline-flex items-center justify-center px-2.5 py-1 rounded-lg bg-blue-500/10 text-blue-400 font-semibold text-xs">
                    {(() => {
