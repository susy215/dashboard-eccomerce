// Panel de notificaciones con polling HTTP
import { Bell, X, ShoppingCart, CreditCard, AlertCircle, Package, Clock, RefreshCw, CheckCircle } from 'lucide-react'
...
export default function NotificationPanel({ isOpen, onClose }) {
  ...
  const PollingIcon = isLoading ? RefreshCw : CheckCircle
      ...
      <div className="relative ml-auto w-full max-w-md bg-white dark:bg-slate-900 shadow-2xl flex flex-col h-full transform transition-transform duration-300 ease-out">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                <PollingIcon className={`w-3 h-3 ${isLoading ? 'text-yellow-500 animate-spin' : 'text-green-500'}`} />
                <span>
                  HTTP Polling • {isLoading ? 'Verificando...' : 'Activo'}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="px-4 py-2 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
          <p className="text-xs text-slate-500 dark:text-slate-400 text-center">
            {notifications.length} notificaciones • Polling cada 30s • Última verificación: {lastChecked ? lastChecked.toLocaleTimeString() : 'Nunca'}
          </p>
        </div>
        ...
        {/* Footer con info de polling */}
        <div className="p-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50">
          <div className="text-center text-xs text-slate-500 dark:text-slate-400">
            🔔 Notificaciones push activas • Polling HTTP cada 30 segundos
          </div>
        </div>
      </div>
    </div>
